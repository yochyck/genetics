...
export interface PedigreePerson { id: string; name: string; sex: 'male' | 'female' | 'unknown'; status: 'healthy' | 'affected' | 'carrier' | 'unknown'; generation: number; x: number; y: number; deceased?: boolean; proband?: boolean; }
export interface PedigreeRelation { id: string; type: 'partner' | 'parent_child'; from: string; to: string; }
export interface Pedigree { id: string; name: string; persons: PedigreePerson[]; relations: PedigreeRelation[]; createdAt?: number; updatedAt?: number; }
...
export interface SimulatorResult { id?: string; input?: InheritanceSimulationInput; gametes1: string[]; gametes2: string[]; punnettSquare: { g1: string; g2: string; genotype: string }[][]; genotypeRatios: Record<string, number>; phenotypeRatios: Record<string, number>; explanation: string; warning?: string; createdAt?: number; }
